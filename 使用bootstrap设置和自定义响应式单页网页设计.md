...
3. 位于freeme / js目录中的所有JavaScript文件已复制到/cloudstreetmarket-webapp/src/main/webapp/js目录。

4. 位于freeme / css目录中的所有CSS文件已复制到/cloudstreetmarket-webapp/src/main/webapp/css目录。
...
8. 此时，使用Web浏览器调用[http:// localhost:8080/portal/index](http://localhost:8080/portal/index)与你之前看到的视图完全相同，但是由我们的应用程序提供。
...
<!-- start: Footer Menu Logo -->
<div class="span1">
    <div class="logoSmall">CLOUD<span class="sub">ST</span><span>M!</span></div>
</div>
<!-- end: Footer Menu Logo -->
<!-- start: Footer Menu Links-->
<div class="span10" >
...
2.在cloudstreetmarket-webapp \ src \ main \ webapp \ js目录中，morris.min.js和raphael.js库已从其各自的网站复制和粘贴。
...
4. Still in index.jsp, under the &lt;!-- start: CSS --&gt; comment, the FeedEk css document has been added:&lt;link href="css/FeedEk.css" rel="stylesheet"&gt;

5. In the style.css file, before the first media query definition \(@media only screen and \(min-width: 960px\)\), the following style definitions have been added

1. 这里的部分使用feedEk jQuery插件。 它有自己的CSS和JavaScript文件。
...
4. 仍然在index.jsp中，在&lt;!-- start: CSS --&gt;注释下，添加了FeedEk css文档：`<link href =“css / FeedEk.css”rel =“stylesheet”>`

5. 在style.css文件中，在第一个媒体查询定义（@media only screen和（min-width：960px））之前，添加了以下样式定义：
...
>
> Bootstrap核心功能在引导中自包含。  js，bootstrap.css和bootstrap-responsive.css。 你真的不应该直接调整这些文件。
...
>
> 如果你不熟悉此标记，其主要目的是在文档中定义设备特定的大小。 从这些大小中，为定向特定和特定于设备的呈现定义规则。 绑定到样式定义的这些规则称为媒体查询。 你可以在style.css文件中找到媒体查询的示例：
>
...
偏移列允许你创建与一个或多个不可见列相对应的固定大小的衰减。 例如，考虑以下代码段：
...
> These icons are normally available commercially but they are also usable for free as part of the Bootstrap product. However Bootstrap asks us to provide an optional backlink to [http://glyphicons.com](http://glyphicons.com).
>
> 这些图标通常是商业上可用的，但它们也可作为Bootstrap产品的一部分免费使用。 但是Bootstrap要求我们提供一个可选的反向链接到[http://glyphicons.com。](http://glyphicons.com。)
...
我们已经为market activity overview实现了一个Bootstrap表。 我们基本上塑造了下表：
