...
> In our application, we have registered custom WebContentInterceptors to override the behaviors of the default one.
>
...
cloudstreetmarket-webapp中也有类似的CloudstreetWebAppWCI：
...
7.最后，在dispatcher-context.xml中，RequestMappingHandlerAdapter bean已被赋予synchronizeOnSession属性：
...
WebContentInterceptor，处理请求提供了控制应用程序应如何与HTTP会话进行反应的能力。
...
WebContentInterceptor通过WebContentGenerator提供setRequireSession（布尔）方法。 这允许定义在处理请求时是否需要会话。
...
另一个有趣的特性来自于synchronizeOnSession属性，我们在RequestMappingHandlerAdapter定义中设置为true。 当设置为true时，会话对象被序列化，并且在同步块中进行访问。 这允许并发访问相同的会话，并避免使用多个浏览器窗口或选项卡时有时会出现的问题。
...
使用我们在CloudstreetWebAppWCI和CloudstreetApiWCI的构造函数中使用的setCacheSeconds（int）方法;  WebContentInterceptor与WebContentGenerator可以管理与缓存相关的几个HTTP响应头。 设置为零，它在响应中添加额外的头部，例如Pragma，Expires，Cache-control等。
...
由于本地的WebContentInterceptor.preHandle方法，我们所有的静态资源都以这种方式缓存24小时。
...
这不是因为我们已经使DefaultController扩展了一个拦截器，拦截器在Controller @RequestMapping路径上作用。 拦截器的注册只通过配置。 如果覆盖路径映射需要修改，我们可以通过以下方式覆盖我们的注册：
...
我们还覆盖了WebContentInterceptor方法的preHandle，postHandle和afterCompletion。 它将允许我们稍后在Controller请求处理之前和之后定义常见的业务相关操作。
...
再次，WebContentGenerator是WebContentInterceptor的超类。 从其JavaDoc页面：http//docs.spring.io/spring/docs/current/javadoc-api/org/ springframework / web / servlet / support / WebContentGenerator.html，您可以找到以下示例：
...
* 通过getServletContext（）访问请求或响应对象中的ServletContext。

* 访问当前Web应用程序的临时目录，由servlet容器通过getTempDir（）提供。

* 通过getWebApplicationContext（）访问WebApplicationContext。



We quickly passed through web caching. There are a lot of customizations and standards in this domain. Also, a new RequestMappingHandlerAdapter has been created with Spring MVC 3.1. It will be helpful to understand the change.

我们很快就通过了网络缓存。 有很多自定义和标准这个域名。 此外，使用SpringMVC 3.1创建了一个新的RequestMappingHandlerAdapter。 这将有助于了解这种变化。
...
https://www.mnot.net/cache\_docs

**New support classes for @RequestMapping since Spring MVC 3.1**

We have used the RequestMappingHandlerAdapter with its bean definition in dispatcher-context.xml. This bean is a new feature with Spring MVC 3.1and has replaced the former AnnotationMethodHandlerAdapter. Also, the support class DefaultAnnotationHandlerMapping has now been replaced by RequestMappingHandlerMapping.

We will go deeper into RequestMappingHandlerAdapter in Chapter 4, Building a REST API for a Stateless Architecture.
...
我们使用了带有它的bean定义的RequestMappingHandlerAdapter dispatcher-context.xml。 这个bean是Spring MVC 3.1的一个新功能并替换了前者的AnnotationMethodHandlerAdapter。 另外，支持类DefaultAnnotationHandlerMapping现在已经被替换RequestMappingHandlerMapping。

我们将在第4章，构建REST中更深入地讨论RequestMappingHandlerAdapter无状态体系结构的API。
