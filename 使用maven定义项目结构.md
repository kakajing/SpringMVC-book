...
1. 从Eclipse，导航到File \| New \| Other.。

2. 将打开一个新向导，你可以select the type of project within a hierarchy。 然后，打开Maven category，选择Maven Project，然后单击Next。
...
1. [ ] edu.zipcloud.cloudstreetmarket as **Group Id          
   **

2. [ ] cloudstreetmarket-parent as **Artifact Id          
   **

3. [ ] 0.0.1-SNAPSHOT as **Version          
   **

4. [ ] pom as **Packaging          
   **

5. [ ] CloudStreetMarket Parent as **Name          
   **
...
3.确保选中**Create a simple project**选项。 单击**Next**。
...
1. [ ]  Double-click on the **pom.xml** file. The **m2eclipse Overview** tab shows up by default. You have to click on the last tab,** pom.xml**, to access the full XML definition.

2. [ ] In this definition, add the following block at the end but still as part of the &lt;project&gt; node. \(You can also copy/paste this piece of code from the cloudstreetmarket-parent's pom.xml of the chapter\_1 source code\):

> You have probably noticed the **maven-surefire-plugin** declaration as well. We will review it soon; it allows us to run unit tests during.
...
> 你可能已经注意到maven-surefire-plugin声明。 我们会尽快进行审查; 它允许我们在运行单元测试。
...
```
输入edu.zipcloud.cloudstreetmarket作为Group Id。

输入0.0.1-SNAPSHOT作为Version。

选择war作为Packaging。

输入CloudStreetMarket Webapp作为Name。
...
2. 在**New Maven Module**向导中，输入以下条目：
   ...
   ```
   选中Create a simple project选项。

       输入cloudstreetmarket-api作为Module Name。

       输入cloudstreetmarket-parent作为Parent project。
   ```
...
5. In the **New Maven Module** wizard, enter the following entries:
   ...
   1. Click on the **Next **button to go to the next step. Fill in the fields with the following:
...
选中Create a simple project选项。
 ...
 输入cloudstreetmarket-core作为Module Name。
...
输入edu.zipcloud.cloudstreetmarket作为Group Id。
 ...
 这一次，选择jar作为Packaging。

 输入CloudStreetMarket Core为Name。
...
1. Now, we will create a company-specific project and its module\(s\):
...
```
输入edu.zipcloud作为Group Id。

   输入zipcloud-parent作为Artifact Id。

  输入0.0.1-SNAPSHOT作为Version。

 选择pom作为Packaging。

 输入ZipCloud Factory Business Parent为Name。
```
...
输入zipcloud-core作为Module Name。

输入zipcloud-parent作为Parent project。
```
...
在我们的研究案例中，将使用m2eclipse修改的运行方式菜单启动它：右键单击zipcloud-parent，然后单击**Run As\|  Maven Clean**。

> In the Maven console, you should now see this beautiful line at the bottom:
>
> 在Maven控制台中，现在应该在底部看到这条美丽的线条：
>
> \[INFO\] BUILD SUCCESS
...
我们刚完成的项目创建屏幕也来自m2eclipse插件。这些屏幕用于使用预配置的pom.xml文件和标准目录结构初始化Java项目。

m2eclipse插件还提供了一组快捷方式来运行Maven构建阶段和一些方便的选项卡（已经看到）来管理项目依赖项并可视化pom.xml配置。

### The standard project hierarchy     ** **

Navigating through the created projects, you should be able to notice a recurring hierarchy made of the following directories: src/main/java, src/main/resource, src/test/java, and src/test/resource. This structure is the default structure that Maven drives  
 us through. This model has become a standard nowadays. But, we can still override it \(in the pom.xml files\) and create our own hierarchy.
...
浏览已创建的项目，您应该能够注意到以下目录的重复层次结构：src / main / java，src / main / resource，src / test / java和src / test / resource。 这个结构是Maven驱动我们通过的默认结构。 这种模式已经成为当今的标准。 但是，我们仍然可以覆盖它（在pom.xml文件中）并创建我们自己的层次结构。
...
These lines allow Maven to use an external JDK for the compiler. It is better to have controlover which compiler Maven uses, especially when managing different environments. Also, there were the following two lines that might look like an over configuration:

这些行允许Maven使用外部JDK作为编译器。 最好是控制Maven使用哪个编译器，特别是在管理不同的环境时。 此外，以下两行可能看起来像一个过度配置：
...
从严格的Maven角度来看，当使用指定的compilerVersion定义外部JDK时，这些行是可选的。 最初，使用这两行代码，我们可以控制我们希望编译默认代码的Java版本。保留旧系统时，现有代码仍然可以在以前的Java版本中编译。

实际上，m2eclipse专门期望这两行，以便将JRE系统库\[JavaSE-1.8\]添加到jar和war模块的构建路径。 现在，使用这些行，Eclipse以与Maven相同的方式编译这些项目：在Java SE 8中。
...
最后，如果打开父项目的`pom.xml`文件，你应该看到用创建的子模块填充的`<modules>`节点。 这已经由m2eclipse自动完成。 我们建议您关注此功能，因为m2eclipse不会总是更新这些`<modules>`节点，具体取决于您更改项目层次结构的方式。
...
在默认生命周期中，你可以找到处理源代码生成，编译，资源处理，测试，集成测试和工件部署的最有趣的构建阶段。有关缺省生命周期中包括的阶段的一些详细信息：
